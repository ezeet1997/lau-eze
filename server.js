...
app.get("/api/ranking", requireLogin, (req, res) => {
  const users = db.prepare("SELECT username, display_name AS displayName, points FROM users ORDER BY points DESC").all();
...
app.post("/api/game-score", requireLogin, (req, res) => {
  ...
  db.prepare("INSERT INTO game_scores (user_id, game, score) VALUES (?, ?, ?)").run(user.id, game, score);
  addPoints(user.id, points);
...
app.listen(PORT, () => {
  console.log(`Laura vs Eze running on port ${PORT}`);
});
